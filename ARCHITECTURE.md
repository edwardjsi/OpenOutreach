...
1. **`handle_connect`** — Unified via `ConnectStrategy` dataclass. Regular: `find_candidate()` from `pools.py`; freemium: `find_freemium_candidate()`. Unreachable detection after `MAX_CONNECT_ATTEMPTS` (3). Transient `SkipProfile` (top card missing after `find_top_card`'s 8s wait) demotes the Deal back to QUALIFIED and only FAILs after `MAX_CONNECT_ATTEMPTS`; a page snapshot is saved when `DUMP_PAGES` is on.
2. **`handle_check_pending`** — Per-profile. Exponential backoff with jitter. On acceptance → enqueues `follow_up`. On `SkipProfile` keeps the Deal PENDING (reconcile re-creates the check) instead of failing it.
...
- **`browser/nav.py`** — Navigation, auto-discovery, `goto_page()`, `extract_in_urls()` (single atomic `evaluate_all` snapshot — no per-element re-resolution). `TOP_CARD_SELECTORS` + `find_top_card()` (waits up to 8s, cycles selectors) target the current profile layout: `section[aria-label="Primary content"] section` (hashed-class SDUi pages) plus legacy selectors; verified against live dumps in `tests/fixtures/pages/connect/`. Checkpoint detection: when the browser lands on `/checkpoint/challenge/`, `await_checkpoint_resolution()` calls `notify_checkpoint()` once, sets `SiteConfig.daemon_halt` (circuit breaker), and returns — the daemon idles with zero requests until the operator solves the challenge via VNC, clears the flag in Admin, and restarts. No auto-resume.
...
- **`notifications.py`** — `notify_checkpoint(session)`: terminal bell + colored banner (always fires) + optional Telegram push via Bot API (`_send_telegram`). Called once per checkpoint from `await_checkpoint_resolution`. Never raises — network errors logged at WARNING. Telegram config via `SiteConfig.telegram_bot_token` + `telegram_chat_id`.
...
- **`exceptions.py`** — `AuthenticationError`, `TerminalStateError`, `SkipProfile`, `ReachedConnectionLimit`, `CheckpointChallengeError`.
- **`onboarding.py`** — Interactive setup.
- **`agents/follow_up.py`** — Follow-up agent. Single LLM call with structured output (`FollowUpDecision`). Conversation is read in Python and injected into the prompt. No tool-calling loop. The prompt carries hard honesty constraints — LinkedIn messages only, never claim email/out-of-band actions, surface `linkedin_username` as the contact email — and binds `[Me]`→`self_name` in the verbatim transcript.
...
- **`SiteConfig`** (DB singleton) — `llm_provider` (required, defaults to `openai`; choices: `openai`/`anthropic`/`google`/`groq`/`mistral`/`cohere`/`openai_compatible`), `llm_api_key` (required), `ai_model` (required), `llm_api_base` (required only for `openai_compatible`). `telegram_bot_token` + `telegram_chat_id` (optional — Telegram push when daemon blocks on a LinkedIn checkpoint, a shift-end activity report via `notifications.send_daily_report(since=shift_start)`, and a final push when the daemon exits: full report via `send_daily_report(..., stopped=True)` from the atexit hook in `linkedin/daemon.py` if the shift hadn't completed, else a brief `notify_daemon_stopped()` note). `daemon_halt` + `daemon_halt_reason` (circuit breaker — set automatically on checkpoint; the daemon idles with zero requests until cleared in Admin and restarted). `enable_active_hours` + `work_shift_hours` (work-shift schedule). Editable via Django Admin.
...
- **Schedule** (via `SiteConfig` model, editable in Django Admin) — `enable_active_hours` (`True`), `work_shift_hours` (2). When enabled, the daemon works for `work_shift_hours` from each start (CLOCK_BOOTTIME — suspend time counts), then idles until restarted; unchecked = 24/7.
- **`conf.py:CAMPAIGN_CONFIG`** — `min_ready_to_connect_prob` (0.9), `min_positive_pool_prob` (0.20), `connect_delay_seconds` (10), `connect_no_candidate_delay_seconds` (300), `check_pending_recheck_after_hours` (36 — raised from 24 after the Aug 2026 restriction incident), `check_pending_jitter_factor` (0.2), `qualification_n_mc_samples` (100), `enrich_min_delay_seconds` (25 — raised from 6), `enrich_max_delay_seconds` (40 — raised from 10), `enrich_max_per_page` (4 — lowered from 10), `burst_min_seconds` (2700), `burst_max_seconds` (3900), `break_min_seconds` (600), `break_max_seconds` (1200), `min_action_interval` (120), `embedding_model` ("BAAI/bge-small-en-v1.5").
