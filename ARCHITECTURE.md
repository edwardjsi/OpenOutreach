...
- **`SiteConfig`** (DB singleton) — `llm_provider` (required, defaults to `openai`; choices: `openai`/`anthropic`/`google`/`groq`/`mistral`/`cohere`/`openai_compatible`), `llm_api_key` (required), `ai_model` (required), `llm_api_base` (required only for `openai_compatible`). `telegram_bot_token` + `telegram_chat_id` (optional — Telegram push when daemon blocks on a LinkedIn checkpoint, plus a shift-end activity report via `notifications.send_daily_report(since=shift_start)`). `daemon_halt` + `daemon_halt_reason` (circuit breaker — set automatically on checkpoint; the daemon idles with zero requests until cleared in Admin and restarted). `enable_active_hours` + `work_shift_hours` (work-shift schedule). Editable via Django Admin.
